...
export function BarberView({ showToast }: { showToast: (msg: string) => void }) {
  ...
  const addDrink = async (id: string, current: number = 0) => {
    const amount = prompt("¿Precio de la bebida?", "100")
    if (amount) {
      try {
  ...
  const generateDayReport = () => {
    const today = new Date()
    const dNames = ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"]
    const currentDayName = dNames[today.getDay()]
    const daySchedule = schedule[currentDayName]

    if (!daySchedule || !daySchedule.enabled) return alert("Hoy estamos cerrados.")

    const todayAppts = appointments.filter(a => a.date.includes("Hoy") || a.date.includes(currentDayName))
    
    let report = `💈 *AGENDA DE HOY - ALCALA BARBER DRINK* 🥃\n`
    report += `📅 ${dateTag}\n\n`
    report += `Estos son los horarios disponibles:\n`

    // Generar huecos (Lógica simple de 30 min)
    const [hS, mS] = daySchedule.start.split(":").map(Number)
    const [hE, mE] = daySchedule.end.split(":").map(Number)
    let currentH = hS, currentM = mS
    
    while (currentH * 60 + currentM < hE * 60 + mE) {
      const timeStr = `${String(currentH % 12 || 12).padStart(2, "0")}:${String(currentM).padStart(2, "0")} ${currentH >= 12 ? 'PM' : 'AM'}`
      const isOccupied = todayAppts.some(a => a.time === timeStr && a.status !== "cancelled")
      report += `${isOccupied ? "❌" : "🟢"} ${timeStr} ${isOccupied ? "-(Ocupado)-" : "*DISPONIBLE*"}\n`
      
      currentM += 30
      if (currentM >= 60) { currentH++; currentM = 0; }
    }

    report += `\nReserva aquí: ${window.location.origin}\n`
    report += `¡Te esperamos para un corte y un drink! ✂️🥃`
    ...
    <div className="max-w-[1100px] mx-auto px-8 py-10">
      ...
      {tab === "horario" && (
        <div className="max-w-[850px]">
          <div className="bg-ink-2 border border-rule rounded-sm p-8 mb-6 flex justify-between items-center">
            <div><h3 className="text-xs uppercase tracking-widest text-gold mb-1">Precio Hora Extra</h3><p className="text-[10px] text-dim">Monto adicional por citas en horarios especiales.</p></div>
          ...
          <button onClick={saveAllConfig} disabled={isSaving} className="w-full py-4 bg-white text-ink text-[11px] uppercase font-black rounded-sm tracking-widest">{isSaving ? "Guardando..." : "Guardar Configuración de Horario"}</button>
        </div>
      )}
      ...
      {tab === "finanzas" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-ink-2 p-10 border border-rule rounded-sm text-center shadow-2xl relative overflow-hidden">
            ...
            <div className="text-[11px] uppercase text-dim tracking-[0.3em] mb-6">Ganancia Total Acumulada</div>
            ...
            <div className="text-xs text-dim italic">Contabilizando {appointments.filter(a => a.status === "attended").length} servicios finalizados.</div>
          </div>
          <div className="bg-ink-2 p-10 border border-rule rounded-sm space-y-6">
            <h3 className="text-xs uppercase tracking-widest text-white border-b border-rule pb-4">Detalle de Ingresos</h3>
            <div className="flex justify-between items-center"><span className="text-dim text-sm">Servicios (Cortes)</span><span className="text-white font-mono text-lg">${appointments.filter(a => a.status === "attended").reduce((sum, a) => sum + (a.price || 0), 0).toLocaleString()}</span></div>
            <div className="flex justify-between items-center"><span className="text-dim text-sm">Bebidas (Drinks)</span><span className="text-white font-mono text-lg">${appointments.filter(a => a.status === "attended").reduce((sum, a) => sum + (a.drinkCharge || 0), 0).toLocaleString()}</span></div>
            <div className="flex justify-between items-center"><span className="text-dim text-sm">Horas Especiales</span><span className="text-white font-mono text-lg">${appointments.filter(a => a.status === "attended").reduce((sum, a) => sum + (a.extraHourCharge || 0), 0).toLocaleString()}</span></div>
            <button onClick={() => {const val = prompt("Monto de venta rápida:"); if(val) showToast("Venta sumada (Simulado)")}} className="w-full mt-4 py-3 border border-rule rounded-sm text-[10px] uppercase tracking-widest text-dim hover:text-white transition-all">+ Venta Rápida de Bebida</button>
          </div>
        </div>
      )}
      ...
      {tab === "servicios" && (
        <div className="grid grid-cols-1 md:grid-cols-[1fr_350px] gap-10">
          ...
          <div className="bg-ink-2 p-8 border border-rule rounded-sm h-fit shadow-xl">
            <h3 className="text-xs uppercase tracking-[0.2em] text-gold mb-8">Nuevo Servicio</h3>
            <div className="space-y-4">
              <input type="text" placeholder="Nombre" value={newService.name} onChange={e => setNewService({...newService, name: e.target.value})} className="w-full bg-ink-3 border border-rule p-4 text-sm text-white rounded-sm outline-none focus:border-gold/40" />
      ...
      {tab === "config" && (
        <div className="max-w-[700px] bg-ink-2 border border-rule p-10 rounded-sm space-y-10">
          <div className="space-y-6">
            <h3 className="text-xs uppercase tracking-widest text-gold border-b border-rule pb-4">Marca y Logo</h3>
            <div className="flex flex-col gap-2">
              <label className="text-[10px] text-dim uppercase tracking-tighter">Enlace de tu imagen (Logo)</label>
              <input type="text" value={logoUrl} onChange={e => setLogoUrl(e.target.value)} placeholder="Ej: https://imgur.com/logo.png" className="bg-ink-3 border border-rule p-4 text-sm text-white rounded-sm outline-none" />
            </div>
          ...
          <div className="space-y-6 pt-4">
            <h3 className="text-xs uppercase tracking-widest text-gold border-b border-rule pb-4">Mensaje de WhatsApp</h3>
            <div className="flex flex-col gap-2">
              <label className="text-[10px] text-dim uppercase tracking-tighter">Plantilla de recordatorio</label>
              <textarea value={reminderMessage} onChange={e => setReminderMessage(e.target.value)} className="bg-ink-3 border border-rule p-5 text-xs text-bright leading-relaxed rounded-sm min-h-[120px] outline-none focus:border-gold/30" />
              <div className="text-[9px] text-dim font-mono">Etiquetas disponibles: [Cliente] [Fecha] [Hora]</div>
            </div>
          </div>
          <button onClick={saveAllConfig} disabled={isSaving} className="w-full py-5 bg-white text-ink text-[10px] uppercase font-black tracking-widest rounded-sm">Guardar Todo</button>
        </div>
      )}
    </div>
  )
